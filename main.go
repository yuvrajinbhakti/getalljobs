...
func (js *JobScraper) ScrapeJobs(searchURL string) error {
	...
	for _, card := range jobCards {
		titleElement, err := card.FindElement(selenium.ByCSSSelector, ".base-search-card__title")
		if err != nil {
			log.Printf("Failed to find job title: %v", err)
			continue
		}
		title, err := titleElement.Text()
		if err != nil {
			log.Printf("Failed to extract text for job title: %v", err)
			continue
		}

		companyElement, err := card.FindElement(selenium.ByCSSSelector, ".base-search-card__subtitle")
		if err != nil {
			log.Printf("Failed to find company name: %v", err)
			continue
		}
		company, err := companyElement.Text()
		if err != nil {
			log.Printf("Failed to extract text for company name: %v", err)
			continue
		}

		locationElement, err := card.FindElement(selenium.ByCSSSelector, ".job-search-card__location")
		if err != nil {
			log.Printf("Failed to find job location: %v", err)
			continue
		}
		location, err := locationElement.Text()
		if err != nil {
			log.Printf("Failed to extract text for job location: %v", err)
			continue
		}
